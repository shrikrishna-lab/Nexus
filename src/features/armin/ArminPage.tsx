  ...
  return (
    <div>
      <ModuleHeader
        ...
        actions={
          <>
            <Button><SplitSquareHorizontal size={16} /> Split View</Button>
          ...
          <div className="border-t border-white/10 p-4">
            ...
            <div className="mt-3 flex flex-wrap justify-between gap-2">
              <div className="flex gap-2">
                <Button><Mic size={16} /> Voice</Button>
                <Button><Code2 size={16} /> Sandbox</Button>
              </div>
              <Button variant="primary"><Send size={16} /> Send</Button>
            </div>
          </div>
        </section>
        <aside className="glass rounded-lg p-4">
          ...
          <div className="mt-3 grid gap-2">
            {["create_task", "search_stash", "open_workspace", "copy_prompt", "log_progress"].map((tool) => (
              <button key={tool} className="focus-ring rounded-md border border-white/10 bg-white/[0.035] px-3 py-2 text-left font-mono text-sm text-cyan-100/72 hover:bg-white/[0.07]">{tool}</button>
            ))}
          </div>
